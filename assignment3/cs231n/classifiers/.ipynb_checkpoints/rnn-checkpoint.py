...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Compoute loss and do backprop
        loss, dscores = temporal_softmax_loss(scores, captions_out, mask, verbose=False)
        dh, dW_vocab, db_vocab = temporal_affine_backward(dscores, cache_taf)
        if self.cell_type == 'rnn':
    ...
    def sample(self, features, max_length=30):
        ...
        word = self._start
        x = np.zeros((N,max_length,W_embed.shape[1]))
        scores = np.zeros((N,max_length,W_vocab.shape[1]))
        prev_c = np.zeros((N,int(Wh.shape[1]/4)))
        for t in range(max_length):
            x[:,t,:], cache_embed = word_embedding_forward(word, W_embed)
            if self.cell_type == 'rnn':
                h, cache_rnn = rnn_step_forward(x[:,t,:], h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                h, prev_c, cache_lstm = lstm_step_forward(x[:,t,:], h, prev_c, Wx, Wh, b)
            scores, cache_taf = temporal_affine_forward(h[:,np.newaxis,:], W_vocab, b_vocab)
            scores = scores.reshape(N,W_vocab.shape[1])
